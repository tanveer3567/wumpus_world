...
def check_true_false(KB, alpha, neg_alpha):
    model = getModel(KB)
    symbols = extract(KB)

    for symbol in extract(alpha):
        symbols.add(symbol)
    ...
    entails_alpha = TT_Check_All(KB, alpha, symbols, model)
    negation_entails_alpha = TT_Check_All(KB, neg_alpha, symbols, model)
    return entails_alpha, negation_entails_alpha
...
def extract(expression):
    symbols = set()
    if expression.symbol[0] != '':
        symbols.add(expression.symbol[0])
    else:
        for subexpression in expression.subexpressions:
            symbols.update(extract(subexpression))
    return symbols
...
def TT_Check_All(KB, alpha, symbols, model):
    if not symbols:
        ...
        return TT_Check_All(KB, alpha, symbols[1:], extend(symbols[0], True, model)) and TT_Check_All(KB, alpha, symbols[1:], extend(symbols[0], False, model))
...
def PL_True(expression, model):
    if expression.connective[0].lower() == 'and':
        ...
        return not PL_True(expression.subexpressions[0], model)
    elif expression.connective[0].lower() == 'xor':
        value = True
        for i, subexpression in enumerate(expression.subexpressions):
            if (i == 0):
                value = PL_True(subexpression, model)
                continue
            value = value ^ PL_True(subexpression, model)
        return value
    elif expression.connective[0].lower() == 'if':
        left = PL_True(expression.subexpressions[0], model)
        right = PL_True(expression.subexpressions[1], model)
